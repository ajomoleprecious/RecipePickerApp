...
icon_img = ImageTk.PhotoImage(file="assets/RRecipe_logo.png")  # path to your custom icon image
...
app_height = screen_height - 100  # set app height
bg_color = "#3d6466"  # set background color
root.geometry(f"{app_width}x{app_height}+{screen_width // 2 - app_width // 2}+0")  # set geometry of app
...
frame2 = tk.Frame(root, width=app_width, height=app_height, bg=bg_color)  # second frame. Height and width are determined by widgets placed inside
...
def adjust_font_size(text, initial_size, width):
    font_size = initial_size
    while True:
        label = tk.Label(root, text=text, font=("TkMenuFont", font_size))
...
def load_frame1():
    ...
    tk.Label(frame1, text="Ready for your random recipe?",
             ...
             wraplength=app_width - 40).pack()  # create widget with wraplength
    # Button
    tk.Button(frame1,
...
def load_frame2():
    ...
    tk.Label(frame2, text=title,
             bg=bg_color, fg="white", font=("TkHeadingFont", 20),
             wraplength=app_width - 40).pack(pady=25)  # create widget with wraplength
    # Labels for ingredients
    for i in ingredients:
        adjusted_font_size = adjust_font_size(i, 12, app_width - 40)
        tk.Label(frame2, text=i, bg='#28393a', fg="white", font=("TkMenuFont", adjusted_font_size),
                 wraplength=app_width - 40).pack(fill='both', padx=20)  # create widget with adjusted font size and wraplength
    # Button
    tk.Button(frame2,
              text="BACK", bg="#28393a",
              fg="white", font=("TkMenuFont", 14),
    ...
    # Label
    tk.Label(frame2, text="Made by Precious",
             fg="white", bg=bg_color, font=("TkMenuFont", 10)
             ).pack(side="bottom")  # create widget
